...
const connectionString = process.env.SERVICEBUS_CONNECTION_STRING || "Endpoint=sb://moekkel667.servicebus.windows.net/;SharedAccessKeyName=RootManageSharedAccessKey;SharedAccessKey=v3V5EP3GZKsLnHA4mtv17YfRdHmli3UCG+ASbLDQ5pM=";
const queueName = process.env.QUEUE_NAME_WITH_SESSIONS || "request-queue";
...
async function main() {
  ...
  try {
    ...
    await sendMessage(sbClient, listOfScientists[1], "session-2");
    await sendMessage(sbClient, listOfScientists[2], "session-1");
    await sendMessage(sbClient, listOfScientists[3], "session-2");
    ...
    await sendMessage(sbClient, listOfScientists[6], "session-1");
    await sendMessage(sbClient, listOfScientists[7], "session-2");
    await sendMessage(sbClient, listOfScientists[8], "session-1");
